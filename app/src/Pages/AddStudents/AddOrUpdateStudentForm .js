...
const AddOrUpdateStudentForm  = ({
  ...
  useEffect(() => {
    if (isModalOpen && isUpdateOn) {
      ...
      getStudentData(subjectAutoGeneratedId);
    }
  }, [isModalOpen, isUpdateOn]);

  const getStudentData = async (subjectAutoGeneratedId) => {
    try {
      const subject = await getStudentFromDatabase(subjectAutoGeneratedId);
  ...
  const handleUpdate = async () => {
    try {
      const response = await updateStudentInDatabase(subjectAutoGeneratedId, studentData);
