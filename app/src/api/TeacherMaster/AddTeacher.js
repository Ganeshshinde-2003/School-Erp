...
import {
  ...
  serverTimestamp,
  query,
  orderBy, // import serverTimestamp
...
export const addTeacherToDatabase = async (teacherData) => {
  ...
  try {
    ...
    // Create subcollections within the teacher document
    const personalDetailsRef = collection(teacherDoc, "PersonalDetails");
    const addressDetailsRef = collection(teacherDoc, "AddressDetails");
    const salaryDetailsRef = collection(teacherDoc, "SalaryDetails");
    const experienceDetailsRef = collection(teacherDoc, "ExperienceDetails");

    // Add data to subcollections (modify with actual data as needed)
    // Personal Details data
    try {
      await addDoc(addressDetailsRef, teacherData?.addressDetailsData);
    } catch (error) {
      console.error("Error adding AddressDetails: ", error);
    }

    try {
      await addDoc(personalDetailsRef, teacherData?.personalDetailsData);
    } catch (error) {
      console.error("Error adding AddressDetails: ", error);
    }

    try {
      await addDoc(salaryDetailsRef, teacherData?.salaryDetailsData);
    } catch (error) {
      console.error("Error adding AddressDetails: ", error);
    }

    try {
      await addDoc(experienceDetailsRef, teacherData?.experienceDetailsData);
    } catch (error) {
      console.error("Error adding AddressDetails: ", error);
    }
...
export const getTeacherFromDatabase = async () => {
  ...
  try {
    ...
    querySnapshot.forEach(async (doc) => {
      // let salery = doc.get('SalaryDetails');
...
//     dob: new Date(2016, 0, 5), // Date of birth
//     fatherName: "xyz",
//     motherName: "abc",
//     spouseName: "sqrt",
//     sex: "male", // Replace with the actual sex value
//     cast: "Some Cast", // Replace with the actual cast
//     castCategory: "General", // Replace with the actual cast category
//     bloodGroup: "AB+", // Replace with the actual blood group
//   };
